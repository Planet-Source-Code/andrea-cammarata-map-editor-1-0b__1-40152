...
Begin VB.Form Form_Materiali 
   ...
   Begin VB.Frame FrameOpzioniTrasparenza 
      ...
      Begin VB.PictureBox RapMPS 
         ...
         End
      End
      Begin MSComctlLib.Slider Solidit‡NO 
         Height          =   255
         Left            =   1080
         TabIndex        =   12
         Top             =   840
      ...
      Begin VB.CommandButton CambiaNO 
         ...
         Width           =   855
      End
      Begin MSComctlLib.Slider Solidit‡NE 
         Height          =   255
         Left            =   1800
         TabIndex        =   13
         Top             =   840
         ...
         Value           =   5
      End
      Begin MSComctlLib.Slider Solidit‡SO 
         Height          =   255
         Left            =   1080
         TabIndex        =   14
         Top             =   3960
         ...
         Value           =   5
      End
      Begin MSComctlLib.Slider Solidit‡SE 
         Height          =   255
         Left            =   1800
         TabIndex        =   15
         Top             =   3960
...
Option Explicit
Dim CambiamentoSolidit‡ As Boolean
Dim CambiamentoAlpha As Boolean
Dim Attivati As Boolean
'Dichiaro sempre le stesse Tre variabili addette a contenere la quantit‡ di
...
Private Sub AlphaBAnbiente_Scroll()
    ...
    'al programma che dovr‡ aggiornare in tempo reale tutti i rispettivi valori Alpha
...
Private Sub AnnullaAssegnazioneColori_Click()
    'Richiamo la funzione che mi permetter‡ di reimpostare a nulli (neri) i colori di tutti i vertici del muro,pavimento o soffitto
    'selezionato
    Reimposta_tutto
End Sub

Private Sub AnnullaTPM_Click()
    'Se il form Ë stato avviato al fine di settare il materiale di un muro,allora
    If Modalit‡GestioneMateriale = "Muri" Then
        ...
        'che la reimpostazione Ë avvenuta con successo
        CaricaMateriale 1
    Else
        'Richiamo la funzione pubblica Reimposta_materiale, in modo che tutti i parametri di
...
Private Sub CambiaNE_Click()
    'Apro la finestra di dilaogo dal componente ControlloSceltaColori che mi permetter‡ appunto
    ...
    If Modalit‡GestioneMateriale = "Muri" Then
        'Richiamo la funzione che mi servir‡ per convertire il colore appena selezionato in valori RGB, passandogli
...
Private Sub Conferma_Click()
    If Modalit‡GestioneMateriale = "Muri" Then
        If SphereMapping.Value = True Then
            Riga(IndiceLista).Propriet‡ = "SphereMapping"
            Reimposta_tutto
        ElseIf Trasparenza.Value = True Then
            Riga(IndiceLista).Propriet‡ = "Trasparenza"
        ElseIf Normale.Value = True Then
            Riga(IndiceLista).Propriet‡ = "Normale"
            Reimposta_tutto
        End If
    Else
        If SphereMapping.Value = True Then
            SoP(IndiceLista2).CR.Propriet‡ = "SphereMapping"
            Reimposta_tutto
        ElseIf Trasparenza.Value = True Then
            SoP(IndiceLista2).CR.Propriet‡ = "Trasparenza"
        ElseIf Normale.Value = True Then
            SoP(IndiceLista2).CR.Propriet‡ = "Normale"
            Reimposta_tutto
        End If
    End If
End Sub
...
Private Sub Esci_Click()
    'Scarica se stesso, cioË chiude il form di gestione dei materiali (Form_Materiali)
    Unload Me
End Sub
...
Private Sub Form_Load()
    ...
    'del form stesso
    Traduci LinguaS
    'Inizializzo la variabile CambiaMentoSolidit‡ un valore iniziale uguale a False
    CambiamentoSolidit‡ = False
    'Richiamo la funzione addetta al caricamento delle propriet‡
    Carica_propriet‡
    'Se il form Ë stato avviato in modalit‡ "Muri" allora...
    If Modalit‡GestioneMateriale = "Muri" Then
        If RTrim(Riga(IndiceLista).Propriet‡) = "Trasparenza" Then
            CaricaColoriTrasparenza 1
        Else: Reimposta_tutto
        End If
        'Richiamo la funzione addetta al caricamento del materiale.
        'Il parametro passato Ë uguale a 1 per fare in modo che venga caricato solo il
        'materiale del muro selezionato
        CaricaMateriale 1
    Else
        If RTrim(SoP(IndiceLista2).CR.Propriet‡) = "Trasparenza" Then
            CaricaColoriTrasparenza 2
        Else: Reimposta_tutto
        End If
        'Richiamo la funzione addetta al caricamento del materiale.
        'Il parametro passato Ë uguale a 2 per fare in modo che venga caricato solo il
...
Sub Attiva_Elementi_Trasparenza()
    ...
    'Attivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Nord - Ovest
    Solidit‡NO.Enabled = True
    'Attivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Nord - Est
    Solidit‡NE.Enabled = True
    'Attivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Sud - Ovest
    Solidit‡SO.Enabled = True
    'Attivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Sud - Est
    Solidit‡SE.Enabled = True
    ...
    'tutti gli elementi sopraindicati sono gi‡ stati attivati
    Attivati = True
End Sub
...
Sub Disattiva_Elementi_Trasparenza()
    ...
    'Disattivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Nord - Ovest
    Solidit‡NO.Enabled = False
    'Disattivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Nord - Est
    Solidit‡NE.Enabled = False
    'Disattivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Sud - Ovest
    Solidit‡SO.Enabled = False
    'Disattivo lo slider che addetto ad impostare il grado di solidit‡ del vertice posto a Sud - Est
    Solidit‡SE.Enabled = False
    ...
    'tutti gli elementi sopraindicati sono gi‡ stati disattivati
    Attivati = False
End Sub

Private Sub RapAmbiente_Click()
    'Apro il componente che mi permetter‡ di selezionare il colore da assegnare alla
    ...
    If Modalit‡GestioneMateriale = "Muri" Then
        ...
        'Questo parametro, elaborato dalla funzione stessa mi permetter‡ di impostare le
        'rispettive quantit‡ di Rosso, Verde e Blu del colore selezionato alla voce Materiale / Ambiente
    ...
    End If
    'Infine assegno alla PictureBox che rappresenta il colore Ambientale, le rispettive quantit‡ di colore
    'appena assegnate
...
Private Sub RapDiffusa_Click()
    ...
    If Modalit‡GestioneMateriale = "Muri" Then
        ...
        'Questo parametro, elaborato dalla funzione stessa mi permetter‡ di impostare le
        'rispettive quantit‡ di Rosso, Verde e Blu del colore selezionato alla voce Materiale / Diffusa
    ...
    End If
    'Infine assegno alla PictureBox che rappresenta il colore Diffuso, le rispettive quantit‡ di colore
    'appena assegnate
...
Private Sub RapEmissiva_Click()
    ...
    If Modalit‡GestioneMateriale = "Muri" Then
        'Richiamo la funzione Preleva_RGB passandogli come unico valore 0.
        'Questo parametro, elaborato dalla funzione stessa mi permetter‡ di impostare le
        'rispettive quantit‡ di Rosso, Verde e Blu del colore selezionato alla voce Materiale / Emissiva
    ...
    End If
    'Infine assegno alla PictureBox che rappresenta il colore Emissivo, le rispettive quantit‡ di colore
    'appena assegnate
...
Private Sub RapSpeculare_Click()
    ...
    If Modalit‡GestioneMateriale = "Muri" Then
        ...
        'Questo parametro, elaborato dalla funzione stessa mi permetter‡ di impostare le
        'rispettive quantit‡ di Rosso, Verde e Blu del colore selezionato alla voce Materiale / Speculare
    ...
    End If
    'Infine assegno alla PictureBox che rappresenta il colore Speculare, le rispettive quantit‡ di colore
    'appena assegnate
    RapSpeculare.BackColor = ControlloSceltaColori.Color
Annulla:
End Sub

Private Sub Solidit‡NE_Scroll()
    'Assegno alla variabile CambiamentoSolidit‡ il valore boolenao True, in modo da far capire al programma
    'che dovr‡ aggiornare il grado di solidit‡ del rispettivo vertice
    CambiamentoSolidit‡ = True
End Sub

Private Sub Solidit‡NO_Scroll()
    'Assegno alla variabile CambiamentoSolidit‡ il valore boolenao True, in modo da far capire al programma
    'che dovr‡ aggiornare il grado di solidit‡ del rispettivo vertice
    CambiamentoSolidit‡ = True
End Sub

Private Sub Solidit‡SE_Scroll()
    'Assegno alla variabile CambiamentoSolidit‡ il valore boolenao True, in modo da far capire al programma
    'che dovr‡ aggiornare il grado di solidit‡ del rispettivo vertice
    CambiamentoSolidit‡ = True
End Sub

Private Sub Solidit‡SO_Scroll()
    'Assegno alla variabile CambiamentoSolidit‡ il valore boolenao True, in modo da far capire al programma
    'che dovr‡ aggiornare il grado di solidit‡ del rispettivo vertice
    CambiamentoSolidit‡ = True
End Sub
...
Private Sub Timer1_Timer()
    'Se l'option button trasparenza selezionato, cioË se Ë stata assegnata al muro, pavimento o soffitto
    'la propriet‡ di trasparenza,allora richiamerÚ la funzione Attiva_elementi_trasparenza che attiver‡ tutti quegli
    'oggetti che mi serviranno per modificare i colori dei vertici dei muri,pavimenti o soffitti con
    'propriet‡ Trasparenza
    If Trasparenza.Value = True And Attivati = False Then
        Attiva_Elementi_Trasparenza
    'Se invece quest‡ opzione non Ë selezionato, richiamerÚ al contrario quella funzione Disattiva_elementi_trasparenza che
    'disattiver‡ gli stessi oggetti.
    'Sarebbe inutile assegnare i colori ad un muro che non Ë trasparente
    ElseIf Trasparenza.Value = False And Attivati = True Then
        Disattiva_Elementi_Trasparenza
    End If
    'Se il valore di qualche slider indicate il grado di solidit‡ del rispettivo vertice Ë cambiato,allora
    'Verr‡ richiamata la funzione che aggiorner‡ in tempo reale il valore di solidit‡ del rispettivo vertice
    'del muro,pavimento o soffitto selezionato.
    'Infine verr‡ riportata la variabile CambiamentoSolidit‡ al suo valore originario (False)
    If CambiamentoSolidit‡ = True Then
        Assegna_solidit‡
        CambiamentoSolidit‡ = False
    End If
...
Sub Reimposta_Colore(Angolo As Integer)
    If Modalit‡GestioneMateriale = "Muri" Then
        ...
        With Riga(IndiceLista).ColVertici(Angolo)
            'Reimposto la quantit‡ di colore Rosso a 0
            .R = 0
            'Reimposto la quantit‡ di colore Verde a 0
            .G = 0
            'Reimposto la quantit‡ di colore Blu a 0
            .B = 0
        End With
        Select Case Angolo
        Case Is = 0
...
Sub Reimposta_tutto()
    'Richiamo 4 volte la stessa funzione (Reimposta_Colore), passandogli perÚ ogni volta un valore diverso:
    ' - La prima volta gli passerÚ il valore 0 per reimpostare a nullo (nero) il colore del vertice posto a Nord - Ovest del muro,
    '   pavimento o soffitto slezionato e la rispettiva Shape che lo rappresenta
    ' - La seconda volta gli passerÚ il valore 1 per reimpostare a nullo (nero) il colore del vertice posto a Nord - Est del muro,
    '   pavimento o soffitto slezionato e la rispettiva Shape che lo rappresenta
    ' - La terza volta gli passerÚ il valore 2 per reimpostare a nullo (nero) il colore del vertice posto a Sud - Ovest del muro,
    '   pavimento o soffitto slezionato e la rispettiva Shape che lo rappresenta
    ' - La seconda volta gli passerÚ il valore 3 per reimpostare a nullo (nero) il colore del vertice posto a Sud - Est del muro,
    '   pavimento o soffitto slezionato e la rispettiva Shape che lo rappresenta
    Reimposta_Colore 0
    Reimposta_Colore 1
    Reimposta_Colore 2
...
Sub Assegna_solidit‡()
    If Modalit‡GestioneMateriale = "Muri" Then
        'Assegno al vertice posto a Nord - Ovest il livello di solidit‡ selezionato dal rispettivo slider
        Riga(IndiceLista).ColVertici(0).A = Solidit‡NO.Value / 10
        'Assegno al vertice posto a Nord - Est il livello di solidit‡ selezionato dal rispettivo slider
        Riga(IndiceLista).ColVertici(1).A = Solidit‡NE.Value / 10
        'Assegno al vertice posto a Sud - Ovest il livello di solidit‡ selezionato dal rispettivo slider
        Riga(IndiceLista).ColVertici(2).A = Solidit‡SO.Value / 10
        'Assegno al vertice posto a Sud - Est il livello di solidit‡ selezionato dal rispettivo slider
        Riga(IndiceLista).ColVertici(3).A = Solidit‡SE.Value / 10
    Else
        'Assegno al vertice posto a Nord - Ovest il livello di solidit‡ selezionato dal rispettivo slider
        SoP(IndiceLista2).CR.ColVertici(0).A = Solidit‡NO.Value / 10
        'Assegno al vertice posto a Nord - Est il livello di solidit‡ selezionato dal rispettivo slider
        SoP(IndiceLista2).CR.ColVertici(1).A = Solidit‡NE.Value / 10
        'Assegno al vertice posto a Sud - Ovest il livello di solidit‡ selezionato dal rispettivo slider
        SoP(IndiceLista2).CR.ColVertici(2).A = Solidit‡SO.Value / 10
        'Assegno al vertice posto a Sud - Est il livello di solidit‡ selezionato dal rispettivo slider
        SoP(IndiceLista2).CR.ColVertici(3).A = Solidit‡SE.Value / 10
    End If
End Sub
...
Private Sub ValorePotenza_Change()
    'Assegno alla variabile CambiamentoAlpha il valore booleano True, in modo da far capire
    'al programma che dovr‡ aggiornare in tempo reale il rispettivo parametro di Potenza
...
Sub CaricaMateriale(TipoCaricamento As Integer)
    If TipoCaricamento = 1 Then
        ...
        With Riga(IndiceLista).Materiale.Ambiente
            'Setto le variabili R G B con le rispettive quantit‡ di colore presenti
        ...
        'Assegno allo slider che rappresenta il valore di luminosit‡ del parametro Ambiente
        ...
        'Assegno allo slider che rappresenta il valore di luminosit‡ del parametro Diffusa
        'presente nel materiale assegnato al muro selezionato
        ...
        'Assegno allo slider che rappresenta il valore di luminosit‡ del parametro Emissiva
        'presente nel materiale assegnato al muro selezionato
        ...
        'Assegno allo slider che rappresenta il valore di luminosit‡ del parametro Speculare
        'presente nel materiale assegnato al muro selezionato
    ...
    'Nel caso in cui invece il TipoCaricamento Ë diverso da 1,e quindi la Modalit‡GestioneMateriali
    'Ë diverso da "Muri",verranno caricati tutti i valori di tutte le voci che compongono il materiale
...
Sub CaricaColoriTrasparenza(TipoCaricamento As Integer)
    'Dichiaro un indice che mi faciliter‡ le operazioni di assegnazione dei colori
    'alle shape che rappresentano i rispettivi vertici
    Dim K As Integer
    'Se il TipoCaricamento Ë uguale a 1,e quindi la Modalit‡GestioneMateriali
    'Ë uguale a "Muri",verranno caricati tutti i valori di tutte le voci che compongono il materiale
    'del muro selezionato
    If TipoCaricamento = 1 Then
        'Avvio un ciclo che assegner‡ alle shape i colori dei rispettivi vertici
        ...
        Next
    'Nel caso in cui invece il TipoCaricamento Ë diverso da 1,e quindi la Modalit‡GestioneMateriali
    'Ë diverso da "Muri",verranno caricati tutti i colori di trasparenza dei rispettivi vertici
    'del pavimento o soffitto selezionato
    Else
        'Avvio un ciclo che assegner‡ alle shape i colori dei rispettivi vertici
...
Sub Assegna_titolo()
    If Modalit‡GestioneMateriale = "Muri" Then
        ...
        'verr‡ applicato il materiale che verr‡ creato e anche la scritta del LabelInformazioni
        ...
        End If
    ElseIf Modalit‡GestioneMateriale = "Pavimento" Then
        If LinguaS = "Italiano" Then
...
Sub Carica_propriet‡()
    If Modalit‡GestioneMateriale = "Muri" Then
        If RTrim(Riga(IndiceLista).Propriet‡) = "SphereMapping" Then
            SphereMapping.Value = True
        ElseIf RTrim(Riga(IndiceLista).Propriet‡) = "Trasparenza" Then
            Trasparenza.Value = True
            'Richiamo la funzione che attiver‡ tutti quegli oggetti, come i bottoni
            'che mi permetteranno di modificare le opzioni di trasparenza del muro,
            'pavimento o soffitto selezionato
            Attiva_Elementi_Trasparenza
        ElseIf RTrim(Riga(IndiceLista).Propriet‡) = "Normale" Then
            Normale.Value = True
        End If
    Else
        If RTrim(SoP(IndiceLista2).CR.Propriet‡) = "SphereMapping" Then
            SphereMapping.Value = True
        ElseIf RTrim(SoP(IndiceLista2).CR.Propriet‡) = "Trasparenza" Then
            Trasparenza.Value = True
            'Richiamo la funzione che attiver‡ tutti quegli oggetti, come i bottoni
            'che mi permetteranno di modificare le opzioni di trasparenza del muro,
            'pavimento o soffitto selezionato
            Attiva_Elementi_Trasparenza
        ElseIf RTrim(SoP(IndiceLista2).CR.Propriet‡) = "Normale" Then
            Normale.Value = True
        End If
    End If
End Sub
...
Sub Traduci(NuovaLingua As String)
    ...
    'in base alla modalit‡ per cui questo Ë stato avviato
    Assegna_titolo
    'Verifico la lingua con cui dovr‡ essere tradotto il Form_Materiali
